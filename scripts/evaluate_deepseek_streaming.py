#!/usr/bin/env python3
"""
DeepSeek V3 streaming evaluation script via OpenRouter.
...
Usage:
    python evaluate_deepseek_streaming.py input.csv --output results.csv
    python evaluate_deepseek_streaming.py input.csv --dry-run  # Cost estimation only
    python evaluate_deepseek_streaming.py input.csv --perturbation-type none --sample-size 10
"""
...
class DeepSeekStreamingEvaluator(BaseEvaluator):
    """
    DeepSeek V3 streaming evaluator via OpenRouter with structured output.
    ...
    def evaluate_single_scenario(self, row: pd.Series, scenario_idx: int, run_number: int = 1) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                result = {
                    **metadata,
                    f'judgment_deepseek_run_{run_number}': evaluation_data.get('judgment', 'ERROR'),
                    f'explanation_deepseek_run_{run_number}': evaluation_data.get('explanation', ''),
                    f'standardized_judgment_deepseek_run_{run_number}': standardized,
                ...
                if attempt < max_retries - 1:
                    ...
                    result = {
                        **metadata,
                        f'judgment_deepseek_run_{run_number}': 'ERROR',
                        f'explanation_deepseek_run_{run_number}': '',
                        f'standardized_judgment_deepseek_run_{run_number}': 'Error',
                        'temperature_used': self.config.temperature,
                        'raw_response': '',
                        'error': str(e)
                    }
    ...
    def run_evaluation(self, df: pd.DataFrame, run_number: int = 1) -> List[Dict[str, Any]]:
        ...
        if self.config.resume and output_file.exists():
            try:
                ...
                judgment_col = f'judgment_deepseek_run_{run_number}'
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="DeepSeek V3 streaming evaluation via OpenRouter",
...
Examples:
  ...
  python evaluate_deepseek_streaming.py input.csv --dry-run

  # Filter to specific perturbation type
  python evaluate_deepseek_streaming.py input.csv --perturbation-type none

  # 3-fold evaluation (run 3 times)
  python evaluate_deepseek_streaming.py input.csv --runs 3
        """
    )
...
def main():
    ...
    if not args.output:
        args.output = suggest_output_filename(args.input_file, "deepseek", args.model)
    ...
    # Create evaluator
    evaluator = DeepSeekStreamingEvaluator(config, api_key)
